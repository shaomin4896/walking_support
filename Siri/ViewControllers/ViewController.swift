...
extension ViewController: CameraFeedManagerDelegate {
    ...
    func drawAfterPerformingCalculations(onInferences inferences: [Inference], withImageSize imageSize:CGSize) {
        ...
        let LeftClearRect = CGRect(x: 139.78847786784172, y: 296.9507553577423, width: 52.43, height: 392.9521164894104)
        let RightClearRect = CGRect(x: 192.64, y: 296.9507553577423, width: 52.43, height: 392.9521164894104)
        ...
        for inference in inferences {
            ...
            let leftNotClear = LeftClearRect.intersects(convertedRect)
            let rightNotClear = RightClearRect.intersects(convertedRect)
            ...
            if speaking{
                ...
                let speaker = Speaker()
                if leftNotClear && rightNotClear{
                    speaker.speak("前方障礙物請繞道")
                    speaking = true
                }else if leftNotClear{
                    speaker.speak("左前方有障礙物請靠右")
                    speaking = true
                }else if rightNotClear{
                    speaker.speak("右前方有障礙物請靠左")
                    speaking = true
                }else{
                    // clear
                }
